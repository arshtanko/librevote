...
if not status.get('peer_id'):
    raise SystemExit('missing peer_id')
required = [
    'node_name',
    'peer_id',
    'listen_multiaddrs',
...
python3 - "$STATUS" "$ELECTION_BEFORE" "$ELECTION_START" "$ELECTION_AFTER" "$VOTE_CAST" "$ELECTION_VOTED" <<'PY'
import json
import sys

network = json.loads(sys.argv[1])
peer_id = network.get('peer_id')
before = json.loads(sys.argv[2])
started = json.loads(sys.argv[3])
after = json.loads(sys.argv[4])
vote = json.loads(sys.argv[5])
voted = json.loads(sys.argv[6])
...
if before.get('local_voter_id'):
    raise SystemExit(f'local voter reported before election exists: {before}')
for name, status in [('start', started), ('after', after)]:
    ...
    if status.get('local_voter_id') != peer_id or not status.get('local_voter_signable'):
        raise SystemExit(f'missing local voter fields in {name} response: {status}')
    if status.get('eligible_voter_ids') != [peer_id]:
        raise SystemExit(f'election voter allowlist does not match local peer: {status}')
if started.get('election_id') != after.get('election_id'):
    raise SystemExit('election id changed after start')
if vote.get('voter_id') != peer_id or vote.get('choice') != 'yes' or not vote.get('object_id'):
