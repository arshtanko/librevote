...
3. Узел выполняет дешевые envelope и сетевые проверки.
4. Узел сохраняет payload как retained pending object, если envelope checks пройдены.
5. Узел передает объект в domain validation pipeline.
6. Узел перепубликует announcement только после valid или valid_for_tally status.
7. Локальное состояние и tally пересчитываются детерминированно из retained valid/conflicted object log и validation records.
```
...
`scope_id` пустой для `network` scope и обязателен для `election_id` или `trustee_selection_id` scope.

Inventory по `network` scope возвращает root objects `TrusteeSelectionElection` и `AnonymousElection` с пустым `scope_id`. После загрузки root object payload узел извлекает `trustee_selection_id` или `election_id` и запускает sync по соответствующему scope.

Inventory возвращает только objects со статусом `valid`, `valid_for_tally` или `valid_but_conflicted`.
...
- `limit_count` и `limit_bytes` ограничивают response.
- `objects[]` возвращается в порядке `object_ids[]` до достижения `limit_count` или `limit_bytes`.
- missing, evicted или policy-rejected objects возвращаются через `missing_object_ids[]` или `rejected_object_ids[]`.
...
Если один и тот же объект пришел много раз, узел проверяет `object_id` и игнорирует дубликаты, кроме локального статуса `pending_payload_evicted`. Для `pending_payload_evicted` matching payload hash заново сохраняется и объект возвращается в validation queue.
...
```text
1. Запросить inventory по network scope для discovery root objects.
2. Скачать missing root payloads и payloads со статусом `pending_payload_evicted`.
3. Извлечь `trustee_selection_id` и `election_id` из valid root payloads.
4. Открыть sync scopes для известных trustee_selection_id и election_id.
5. Запросить inventory по каждому активному scope_id.
6. Сравнить object_ids с локальным object log и статусом payload retention.
7. Запросить отсутствующие payloads и payloads со статусом `pending_payload_evicted` через get-object или get-objects.
8. Сохранить downloaded payload как retained pending object после дешевых envelope checks.
9. Выполнить domain validation и обновить validation status.
10. Повторить до отсутствия новых object ids или достижения лимитов.
```

Для первой версии не требуется глобальный consensus. Достаточно, чтобы честные узлы со временем получили один и тот же набор валидных объектов при наличии хотя бы одного честного reachable peer.
...
- Объект адресуется по hash canonical object bytes.
...
TrusteeVote: low
TrusteeConsent: low
TallyKeyContribution: medium
TallyKeySet: medium
AnonymousElection: medium/high
AnonymousBallot: low/medium
TallyDecryptionShare: low
SyncRequest: low
```
...
- Лимит request bytes для `get-objects`.
